...
class GANimation(BaseModel):
    ...
    def set_input(self, input):
        ...
        if len(self._gpu_ids) > 0:
            self._input_real_img = self._input_real_img.cuda(int(self._gpu_ids[0]), async=True)
            self._input_real_cond = self._input_real_cond.cuda(int(self._gpu_ids[0]), async=True)
            self._input_desired_cond = self._input_desired_cond.cuda(int(self._gpu_ids[0]), async=True)
